...
static void update_step_1d(const double *X, double *C, const int *assign, int N, int K) {
    ...
    #pragma omp parallel
    {
        ...
        #pragma omp master // O master (thread 0) pega o número total de threads
        n_threads = omp_get_num_threads();
        
        // "Opção A": Acumuladores locais por thread
...
int main(int argc, char **argv){
    ...
    // argumentos para schedule
    const char* schedule_str = (argc > 5) ? argv[5] : "static";
    int chunk_size = (argc > 6) ? atoi(argv[6]) : 0; // 0 significa chunk padrão

    if (strcmp(schedule_str, "static") == 0) {
        omp_set_schedule(omp_sched_static, chunk_size > 0 ? chunk_size : 0);
    } else if (strcmp(schedule_str, "dynamic") == 0) {
        omp_set_schedule(omp_sched_dynamic, chunk_size > 0 ? chunk_size : 1); 
    }
    ...
    if(!assign){ free(X); free(C); return 1; }
